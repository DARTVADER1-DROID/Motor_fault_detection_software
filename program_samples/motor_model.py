...
if __name__ == "__main__":
    print("=" * 80)
    print("12V BRUSHED DC MOTOR - SIMPLE, PRACTICAL MODEL")
    print("=" * 80)

    motor = DCMotor12V("College-Motor")

    # TEST 1: Normal no-load operation
    print("\n[TEST 1] Power ON → Start → No-load running")
    print("-" * 80)
    motor.power_on()
    motor.start()
    
    for i in range(60):
        # Simulate: startup ramp then stable no-load
        if i < 10:
            rpm = i * 300  # Ramp 0-3000 RPM
            current = 0.5 + (0.3 * i / 10.0)  # Ramp 0.5-0.8A
        else:
            rpm = 2800  # Stable no-load speed
            current = 0.6  # Stable no-load current
        
        motor.update(12.0, current, rpm, 25.0)
        
        if i % 15 == 0:
            s = motor.status()
            print(f"Cycle {i:2d}: {s['state']:10s} | RPM={s['speed_rpm']:4d} | I={s['current_a']:.2f}A | Eff={s['efficiency_pct']:.1f}% | Health={s['health_score']:.0f}%")

    # TEST 2: Motor stalls (jam)
    print("\n[TEST 2] STALL - Sudden jam (load applied)")
    print("-" * 80)
    for i in range(30):
        if i < 15:
            # Normal running
            motor.update(12.0, 0.8, 2800, 40.0)
        else:
            # Suddenly jammed
            motor.update(12.0, 6.0, 10, 65.0)
        
        if i >= 10:
            s = motor.status()
            print(f"Cycle {i:2d}: {s['state']:10s} | RPM={s['speed_rpm']:4d} | I={s['current_a']:.2f}A | Faults={s['faults']}")

    # TEST 3: Overheat
    print("\n[TEST 3] OVERHEAT - Temperature rising")
    print("-" * 80)
    motor.power_on()
    motor.start()
    for i in range(50):
        temp = 25.0 + (70.0 * i / 50.0)  # Ramp 25-95°C
        motor.update(12.0, 2.5, 2500, temp)
        if i % 10 == 0:
            s = motor.status()
            print(f"Cycle {i:2d}: T={s['temperature_c']:.1f}°C | Faults={s['faults']} | Health={s['health_score']:.0f}%")

    # TEST 4: Low efficiency (bearing wear)
    print("\n[TEST 4] LOW EFFICIENCY - Bearing friction")
    print("-" * 80)
    motor = DCMotor12V("Worn-Motor")
    motor.power_on()
    motor.start()
    for i in range(25):
        # High current but low speed (bearing friction)
        motor.update(12.0, 3.5, 1000, 45.0)
        if i >= 15:
            s = motor.status()
            print(f"Cycle {i:2d}: Eff={s['efficiency_pct']:.1f}% | Faults={s['faults']}")

    # TEST 5: Brush wear (current ripple)
    print("\n[TEST 5] BRUSH WEAR - High current ripple")
    print("-" * 80)
    motor = DCMotor12V("Aged-Motor")
    motor.power_on()
    motor.start()
    for i in range(40):
        # Simulate ripple in current (bad brush contact)
        base_current = 2.0
        ripple = 0.5 * (i / 40.0)  # Ripple increases
        current = base_current + (0.3 if i % 2 == 0 else -0.3 * ripple)
        
        motor.update(12.0, max(0, current), 2500, 50.0)
        if i % 10 == 0:
            s = motor.status()
            print(f"Cycle {i:2d}: Ripple={s['current_ripple_pct']:.1f}% | Faults={s['faults']}")
